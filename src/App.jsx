 ...
 *
 * NEW in v3.0:
 *  - Multi-role login: admin vs member (members see own bill/meals only)
 *  - Admin name & password change (persisted in localStorage)
 *  - Add / remove admin accounts
 *  - Auto bill PDF export per member + full month report
 *  - Guest meal tracking with cost allocation
 *  - Extra charges (utility, service fee, etc.) per member
...
import {
  addDoc, collection, onSnapshot, updateDoc, deleteDoc,
...
function Avatar({name="?",size=8}) {
  const colors=["bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-400",
...
function generateBillHTML(member,ym,mealRate,bazaarEntries,depositEntries,extraEntries,guestEntries,totalBazaar,totalMeals) {
  ...
  const extraRows=extras.map(e=>`<tr><td>${fmtDate(e.date)}</td><td>${e.note||"Extra charge"}</td><td style="color:#dc2626">-${fmtCurrency(e.amount)}</td></tr>`).join("");
...
// ─── Dashboard ────────────────────────────────────────────────────────────────
function Dashboard({members,meals,bazaar,deposits,extraCharges,guestMeals,currentUser,isAdmin}) {
  ...
  Object.entries(meals).forEach(([key,entry])=>{
    ...
    bT+=entry.breakfast?1:0; lT+=entry.lunch?1:0; dT+=entry.dinner?1:0;
  });
...
// ─── Members ──────────────────────────────────────────────────────────────────
function MembersPage({members,notify}) {
  ...
  const handleSave=async()=>{
    ...
    try{
      ...
      resetForm();
    }catch(err){notify("Firebase error: "+err.message,"error");}
    finally{setBusy(false);}
  };

  const handleDelete=async(id)=>{
    setBusy(true);
    try{await deleteDoc(doc(db,"members",id));notify("Member deleted");}
  ...
  const toggleStatus=async(m)=>{
    ...
    try{
      await updateDoc(doc(db,"members",m.id),{status:newStatus,updatedAt:serverTimestamp()});
                      ...
                      <td className="px-4 py-3">
                        <button onClick={()=>toggleStatus(m)}
                          className={`px-2.5 py-1 rounded-full text-xs font-medium transition-colors
                            ${m.status==="inactive"?"bg-gray-100 dark:bg-gray-800 text-gray-500 hover:bg-emerald-100 hover:text-emerald-700"
...
function MealsPage({members,meals,notify,currentUser,isAdmin}) {
  ...
  const getMealDoc=memberId=>meals[`${selectedDate}_${memberId}`]||{breakfast:false,lunch:false,dinner:false};
  const getMeal=(memberId,type)=>!!getMealDoc(memberId)[type];

  const toggleMeal=async(memberId,type)=>{
    if(!isAdmin&&memberId!==currentUser.memberId) return;
    const key=`${selectedDate}_${memberId}`;
    const current=getMealDoc(memberId);
    const newVal=!current[type];
    const busyId=`${key}_${type}`;
    setBusyKey(busyId);
    try{
      await setDoc(doc(db,"meals",key),{
  ...
  const setAllMeals=async(type,val)=>{
    ...
    try{
      await Promise.all(visibleMembers.map(m=>{
        const key=`${selectedDate}_${m.id}`;
        const cur=getMealDoc(m.id);
        return setDoc(doc(db,"meals",key),{
          date:selectedDate,memberId:m.id,
                                ...
                                <td key={type} className="px-4 py-3 text-center">
                                  <button onClick={()=>toggleMeal(m.id,type)} disabled={isBusy}
                                    className={`w-12 h-10 rounded-xl text-sm font-bold transition-all
                                      ${isOn?"bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 shadow-sm scale-105"
...
// ─── Bazaar ───────────────────────────────────────────────────────────────────
function BazaarPage({bazaar,notify,currentUser}) {
  ...
  const handleSave=async()=>{
    ...
    try{
      if(editId){
        await updateDoc(doc(db,"bazaar",editId),{date:form.date,amount:amt,note:form.note,updatedAt:serverTimestamp()});
        notify("Entry updated");
      }else{
        await addDoc(collection(db,"bazaar"),{date:form.date,amount:amt,note:form.note,addedBy:currentUser.name,createdAt:serverTimestamp()});
  ...
  const handleDelete=async(id)=>{
    setBusy(true);
    try{await deleteDoc(doc(db,"bazaar",id));notify("Entry deleted");}
    catch(err){notify("Delete failed: "+err.message,"error");}
    finally{setBusy(false);setConfirm(null);}
  };
...
// ─── Deposits ─────────────────────────────────────────────────────────────────
function DepositsPage({members,deposits,notify}) {
  ...
  const handleSave=async()=>{
    ...
    setBusy(true);
    try{
      await addDoc(collection(db,"deposits"),{memberId:form.memberId,date:form.date,amount:amt,note:form.note,createdAt:serverTimestamp()});
  ...
  const handleDelete=async(id)=>{
    setBusy(true);
    try{await deleteDoc(doc(db,"deposits",id));notify("Deposit deleted");}
...
// ─── Extra Charges ────────────────────────────────────────────────────────────
function ExtrasPage({members,extraCharges,notify}) {
  const [form,setForm]=useState({memberId:"",date:TODAY,amount:"",note:""});
  const [busy,setBusy]=useState(false);
  const [filter,setFilter]=useState("");
  const [confirm,setConfirm]=useState(null);
  ...
  const handleSave=async()=>{
    ...
    setBusy(true);
    try{
      await addDoc(collection(db,"extraCharges"),{memberId:form.memberId,date:form.date,amount:amt,note:form.note,createdAt:serverTimestamp()});
      setForm(f=>({...f,amount:"",note:""}));
  ...
  const handleDelete=async(id)=>{
    setBusy(true);
    try{await deleteDoc(doc(db,"extraCharges",id));notify("Deleted");}
    catch(err){notify("Delete failed","error");}
    finally{setBusy(false);setConfirm(null);}
  };
...
// ─── Guest Meals ──────────────────────────────────────────────────────────────
function GuestsPage({guestMeals,notify}) {
  ...
  const handleSave=async()=>{
    ...
    setBusy(true);
    try{
      await addDoc(collection(db,"guestMeals"),{...form,mealCount:Number(form.mealCount),createdAt:serverTimestamp()});
  ...
  const handleDelete=async(id)=>{
    setBusy(true);
    try{await deleteDoc(doc(db,"guestMeals",id));notify("Deleted");}
    catch(err){notify("Delete failed","error");}
    finally{setBusy(false);setConfirm(null);}
  };
...
function SettingsPage({dark,setDark,onLogout,notify,currentUser,members}) {
  ...
  const saveProfile=()=>{
    if(!profForm.name.trim()){notify("Name required","error");return;}
    const updated=admins.map(a=>a.id===currentUser.id?{...a,...profForm}:a);
    saveAdmins(updated);
    setAdmins(updated);
    currentUser.name=profForm.name;
    currentUser.email=profForm.email;
    notify("Profile updated — reload to see name in sidebar");
  };

  const savePassword=()=>{
    const me=admins.find(a=>a.id===currentUser.id);
    if(!me||me.password!==pwForm.old){notify("Old password incorrect","error");return;}
    ...
    const updated=admins.map(a=>a.id===currentUser.id?{...a,password:pwForm.new1}:a);
    saveAdmins(updated);
    setAdmins(updated);
  ...
  const addAdmin=()=>{
    if(!newAdmin.name.trim()||!newAdmin.email.trim()||!newAdmin.password.trim()){notify("All fields required","error");return;}
    if(admins.find(a=>a.email===newAdmin.email)){notify("Email already exists","error");return;}
    const updated=[...admins,{...newAdmin,id:uid(),role:"admin"}];
    saveAdmins(updated);
    setAdmins(updated);
  ...
  const saveMemberLogin=()=>{
    if(!memberLoginForm.memberId||!memberLoginForm.password.trim()){notify("Select member and set password","error");return;}
    const member=members.find(m=>m.id===memberLoginForm.memberId);
    if(!member){notify("Member not found","error");return;}
    const existing=memberLogins.filter(l=>l.memberId!==memberLoginForm.memberId);
    const updated=[...existing,{memberId:memberLoginForm.memberId,name:member.name,email:member.email||"",password:memberLoginForm.password}];
    localStorage.setItem(MEMBER_LOGINS_KEY,JSON.stringify(updated));
    setMemberLogins(updated);
    setMemberLoginForm({memberId:"",password:""});
    notify(`Login set for ${member.name}`);
  };
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <Card title="My Profile">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div><label className={labelCls}>Email</label><input type="email" value={profForm.email} onChange={e=>setProfForm(f=>({...f,email:e.target.value}))} className={inputCls}/></div>
        </div>
      ...
      <Card title="Change Password">
        <div className="space-y-3">
          ...
          <BtnPrimary onClick={savePassword}>Change Password</BtnPrimary>
        </div>
      </Card>
      ...
      {isAdmin&&(
        <Card title="Admin Accounts" sub="Manage who can log in as admin">
          <div className="space-y-2 mb-5">
            {admins.map(a=>(
              <div key={a.id} className="flex items-center justify-between py-2.5 px-3 rounded-xl bg-gray-50 dark:bg-gray-800">
                <div className="flex items-center gap-3">
                  <Avatar name={a.name} size={8}/>
                  <div>
                    <p className="text-sm font-medium">{a.name}{a.id===currentUser.id&&<span className="ml-2 text-xs text-indigo-500">(you)</span>}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{a.email}</p>
                  </div>
                </div>
                {a.id!==currentUser.id&&(
                  <button onClick={()=>setConfirm({id:a.id,name:a.name})}
                    className="px-3 py-1 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-xs font-medium hover:bg-red-100">Remove</button>
                )}
              </div>
            ))}
          </div>
          <div className="border-t border-gray-100 dark:border-gray-800 pt-4">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Add New Admin</p>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div><input value={newAdmin.name} onChange={e=>setNewAdmin(f=>({...f,name:e.target.value}))} placeholder="Name" className={inputCls}/></div>
              <div><input type="email" value={newAdmin.email} onChange={e=>setNewAdmin(f=>({...f,email:e.target.value}))} placeholder="Email" className={inputCls}/></div>
              <div><input type="password" value={newAdmin.password} onChange={e=>setNewAdmin(f=>({...f,password:e.target.value}))} placeholder="Password" className={inputCls}/></div>
            </div>
            <BtnPrimary onClick={addAdmin} className="mt-3">Add Admin</BtnPrimary>
          </div>
        </Card>
      )}
      ...
      {isAdmin&&(
        <Card title="Member Login Setup" sub="Give members a password to view their own bill">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className={labelCls}>Password</label>
              <input type="password" value={memberLoginForm.password} onChange={e=>setMemberLoginForm(f=>({...f,password:e.target.value}))} className={inputCls} placeholder="Set login password"/>
            </div>
          </div>
          <BtnPrimary onClick={saveMemberLogin}>Set Member Login</BtnPrimary>
          {memberLogins.length>0&&(
            <div className="mt-4 space-y-2">
              <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">Members with login access:</p>
              {memberLogins.map(l=>(
                <div key={l.memberId} className="flex items-center justify-between py-2 px-3 rounded-xl bg-gray-50 dark:bg-gray-800 text-sm">
                  <span className="font-medium">{l.name}</span>
                  <span className="text-xs text-gray-500 dark:text-gray-400">{l.email||"no email"}</span>
                </div>
              ))}
            </div>
          )}
          <div className="mt-3 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl text-xs text-amber-700 dark:text-amber-400">
            ⚠️ Member logins are stored locally. For production, use Firebase Authentication.
          </div>
        </Card>
      )}
      ...
      <ConfirmDialog open={!!confirm} title="Remove Admin"
        desc={`Remove "${confirm?.name}" from admin accounts?`}
